...
const config = {
  ...
  releaseEmoji: (() => {
    ...
    const id = process.env.RELEASE_EMOJI_ID || '1553445866109411448';
  ...
  // Discord bots can send 2000 characters, not the 4000 Nitro user limit.
  // 0 disables fitting. Anything higher is capped so the bot posts one message.
  fitLimit: (() => {
    const hard = 1990;
    if (process.env.DISCORD_CHAR_LIMIT !== undefined && process.env.DISCORD_CHAR_LIMIT !== '') {
      const n = int(process.env.DISCORD_CHAR_LIMIT, hard);
      if (n === 0) return 0;
